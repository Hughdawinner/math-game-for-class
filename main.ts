    ...
    if (hint_one_was_recieved == false) {
        if (answer1 > 0) {
            if (answer1 % 10 > 0) {
                ...
                game.showLongText("You answered the question correctly you silly goose!", DialogLayout.Center)
            }
        } else {
            game.showLongText("Stop asking for hints before you answer the question", DialogLayout.Center)
        }
    }
})
    ...
    if (The_question_was_received == false) {
        if (mySprite.tileKindAt(TileDirection.Top, assets.tile`myTile2`)) {
            if (controller.A.isPressed()) {
                game.showLongText("Add the two numbers together, than go to the door to the left to type the answer", DialogLayout.Center)
